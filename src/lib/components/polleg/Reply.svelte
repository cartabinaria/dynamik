<script lang="ts">
	...
	let sortedNestedReplies = $derived(allNestedReplies.length > 0
...
<div class="flex gap-4 w-full group" data-reply-id={reply.id} bind:this={replyContainer}>
	...
	<div class="flex-1 min-w-0 mr-4">
		<!-- Main Reply Content -->
		<div class="bg-base-200 backdrop-blur-sm rounded-lg p-4 mb-4 transition-all duration-200 border border-transparent">
			<!-- Reply Header -->
			<div class="flex items-center justify-between mb-4">
				...
				{#if reply.can_i_delete}
					<button
						...
						onclick={(e) => { e.preventDefault(); deleteReply(reply.id); }}
						disabled={isDeleting}
						title="Delete reply"
				...
				{#if user}
					<button
						...
						onclick={(e) => {
							...
							if (showReplyBox) {
								setTimeout(() => {
									const replyBoxElement = document.querySelector(`[data-reply-id="${reply.id}"] .reply-box-container`);
									if (replyBoxElement) {
					...
					>
						<span class="icon-[solar--reply-outline] group-hover/reply:rotate-12 transition-transform"></span>
						Reply
					</button>
				{/if}
			</div>
		...
		{#if showNestedReplies && sortedNestedReplies && sortedNestedReplies.length > 0}
			<div class="ml-8 animate-in slide-in-from-left-4 duration-300">
				{#each sortedNestedReplies as nestedReply, nestedIndex (nestedReply.id)}
					<div class="animate-in fade-in slide-in-from-top-2 duration-300" style="animation-delay: {nestedIndex * 50}ms">
						<Reply
							{answer}
							reply={nestedReply}
							index={nestedIndex}
		...
		{#if showReplyBox}
			<div class="reply-box-container ml-8">
				<div class="bg-base-200/40 backdrop-blur-sm rounded-lg p-4 border border-primary/20 shadow-lg animate-in slide-in-from-top-2 duration-300">
					<ReplyBox
						closeCallback={() => {
							showReplyBox = false;
						}}
